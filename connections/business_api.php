...
$result = $conn->query('SELECT COUNT(*) AS newArticlesCount FROM business_articles WHERE status = 0');
...
if (isset($_POST['refresh'])) {
    ...
    $endpoint = 'https://newsapi.org/v2/top-headlines?country=us&category=business&apiKey=900bd2191c254d00b5650a6741cf2b16';
    $category = 'Business';
    ...
    if ($data['status'] === 'ok') {
        ...
        $stmtArticles = $conn->prepare('INSERT INTO business_articles (title, description, url, content, urlToImage, source, publishedAt, category_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
        ...
        foreach ($articles as $article) {
            ...
            $checkStmt = $conn->prepare('SELECT COUNT(*) FROM business_articles WHERE title = ?');
